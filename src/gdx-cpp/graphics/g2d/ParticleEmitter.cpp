...
void ParticleEmitter::addParticles ( unsigned count )
{
    count = std::min ( ( int ) count, maxParticleCount - activeCount );
    if ( count == 0 ) {
        return;
    }
...
void ParticleEmitter::update ( float delta )
{
    ...
    if ( durationTimer < duration ) {
        ...
        if ( !continuous || allowCompletionVar ) {
            return;
        }
        restart();
    }
...
{
    if ( additive ) {
        spriteBatch.setBlendFunction ( gdx::GL::SRC_ALPHA, gdx::GL::ONE );
    }
    ...
    if ( additive ) {
        spriteBatch.setBlendFunction ( gdx::GL::SRC_ALPHA, gdx::GL::ONE_MINUS_SRC_ALPHA );
    }
}
...
void ParticleEmitter::restart ()
{
    ...
    updateFlags = 0;
    if ( angleValue.active && angleValue.timeline.size() > 1 ) {
        updateFlags |= UPDATE_ANGLE;
    }
    if ( velocityValue.active && velocityValue.active ) {
        updateFlags |= UPDATE_VELOCITY;
    }
    if ( scaleValue.timeline.size() > 1 ) {
        updateFlags |= UPDATE_SCALE;
    }
    if ( rotationValue.active && rotationValue.timeline.size() > 1 ) {
...
//     float offsetTime = lifeOffset + lifeOffsetDiff * lifeOffsetValue.getScale(percent);
    ...
    if ( ( updateFlags & UPDATE_TINT ) == 0 ) {
        if ( particle->tint.size() == 0 ) {
            particle->tint.resize ( 3 );
        }
    ...
    if ( xOffsetValue.active ) {
        x += ( int ) xOffsetValue.newLowValue();
    }
    float y = this->y;
    if ( yOffsetValue.active ) {
        y += ( int ) yOffsetValue.newLowValue();
    }
...
std::string readString ( std::istream& reader, const std::string& name )
{
    ...
    std::size_t found;
    if ( line.length() == 0 ) {
        gdx_log_error ( "gdx", "Missing value: %s", name.c_str() );
    }
    found = line.find ( ":", 0 );
