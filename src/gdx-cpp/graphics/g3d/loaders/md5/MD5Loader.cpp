...
MD5Animation& MD5Loader::loadAnimation (const InputStream& in) {
    ...
    float MD5Loader::parseFloat (const std::string& value) {
        float front = 0;
        float back = 0;
        float sign = 1;
    ...
    int MD5Loader::parseInt (const std::string& value) {
        int front = 0;
        int sign = 1;
    ...
    void MD5Loader::buildFrameSkeleton (const MD5Animation& animation,int frameIndex) {
    ...
    void MD5Loader::tokenize (const std::string& line,std::list<String>& tokens) {
        tokens.clear();
