...
float* MD5Mesh::createVertexArray (int stride) {
...
void MD5Mesh::calculateVerticesJni (const MD5Joints& skeleton) {
    MD5Jni.calculateVertices(skeleton.joints, weights, vertices, verts, numVertices);
}

void MD5Mesh::calculateNormalsBind (const MD5Joints& bindPoseSkeleton) {
...
void MD5Mesh::calculateNormalsBind (const MD5Joints& skeleton,float vertices,const short& indices,float verts,int vstride,int wstride) {
    for (int vertexOffset = 2, k = 0; vertexOffset < vertices.length; vertexOffset += vstride) {
        float finalX = 0;
        float finalY = 0;
        float finalZ = 0;
...
void MD5Mesh::read (const DataInputStream& in) throws IOException {
    shader = in.readUTF();
...
void MD5Mesh::write (const DataOutputStream& out) throws IOException {
    out.writeUTF(shader);
