...
void MD5Quaternion::multiply (const MD5Quaternion& q) {
...
float MD5Quaternion::dot (const MD5Quaternion& q) {
    return x * q.x + y * q.y + z * q.z + w * q.w;
}

void MD5Quaternion::slerp (const MD5Quaternion& q,float t) {
    if (t <= 0) return;
