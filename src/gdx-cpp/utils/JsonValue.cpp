...
JsonValue::JsonValue(int val) : item_type(json_int) {
    item_val = gdx::shared_ptr<void>( new int(val), json_deleter<int>() );
}

JsonValue::JsonValue(int* val) : item_val(gdx::shared_ptr<void>(val, json_deleter<int>())), item_type(json_int) {

}

JsonValue::JsonValue(float val) : item_val(gdx::shared_ptr<void>( new float(val), json_deleter<float>() ) ), item_type(json_float) {

}

JsonValue::JsonValue(float* val) : item_val(gdx::shared_ptr<void>(val, json_deleter<float>())), item_type(json_float)
{
}

JsonValue::JsonValue(bool* val) : item_val(gdx::shared_ptr<void>( val, json_deleter<bool>() )), item_type(json_bool)  {
}

JsonValue::JsonValue(const std::string& val)
    : item_val(gdx::shared_ptr<void>( new std::string(val), json_deleter<std::string>())) , item_type(json_string)
{
}

JsonValue::JsonValue(std::string* val)
    : item_val(gdx::shared_ptr<void>( val , json_deleter<std::string>())),item_type(json_string)
{
}

JsonValue::JsonValue(const item_map& val) : item_type(json_json) {
    item_val = gdx::shared_ptr<void>( new item_map(val), json_deleter<item_map>() );
}

JsonValue::JsonValue(const array& val) : item_type(json_list) {
    item_val = gdx::shared_ptr<void>( new array(val), json_deleter<item_map>() );
}

JsonValue::JsonValue(bool val) : item_val(gdx::shared_ptr<void>(new bool(val), json_deleter<bool>())), item_type(json_bool) {
}
...
JsonValue& JsonValue::at(unsigned int idx)
{
    if (this->item_type == json_null) {
        ...
        this->item_val = gdx::shared_ptr<void>(new_array, json_deleter<array>());
...
{
    if (this->item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new item_map, json_deleter<item_map>());
...
JsonValue::JsonValue(JsonValue::item_map* val) : item_val(gdx::shared_ptr<void>( val , json_deleter<item_map>() )), item_type(json_json) {
    
}

JsonValue::JsonValue(JsonValue::array* val) : item_val(gdx::shared_ptr<void>( val, json_deleter<array>() )), item_type(json_list) {    
}

JsonValue& JsonValue::operator=(const std::string& other) {
    if (item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new std::string(other), json_deleter<std::string>());
...
JsonValue& JsonValue::operator=(int other) {
    if (item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new int(other), json_deleter<int>());
...
JsonValue& JsonValue::operator=(bool other) {
    if (item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new bool(other), json_deleter<bool>());
...
JsonValue& JsonValue::operator=(float other) {
    if (item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new float(other), json_deleter<float>());
...
JsonValue& JsonValue::operator=(const JsonValue::array& other) {
    if (item_type == json_null) {
        this->item_val = gdx::shared_ptr<void>(new array(other), json_deleter<array>());
        this->item_type = json_list;
    } else {
        this->as_array() = other;
    }
