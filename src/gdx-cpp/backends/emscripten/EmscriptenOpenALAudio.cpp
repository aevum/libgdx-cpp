...
EmscriptenOpenALAudio::EmscriptenOpenALAudio (int simultaneousSources)
{
    createAl();
...
void EmscriptenOpenALAudio::createAl() {
...
gdx::Sound * EmscriptenOpenALAudio::newSound (const ref_ptr_maker< gdx::FileHandle >::shared_ptr_t file) {
//     if (file == NULL) gdx_log_error("gdx","file cannot be null.");
    std::string extension = file->extension();
    if (extension == "ogg")
      return new EmscriptenOggSound(this, file);
    else {
...
gdx::Music * EmscriptenOpenALAudio::newMusic (const ref_ptr_maker< gdx::FileHandle >::shared_ptr_t file) {
    std::string extension = file->extension();
    if (extension == "ogg")
      return new EmscriptenOggMusic(this, file);
    else {
...
int EmscriptenOpenALAudio::obtainSource (bool isMusic) {
...
void EmscriptenOpenALAudio::freeSource (int sourceID) {
    CHECK_OPENAL_ERROR(alSourceStop(sourceID));
    CHECK_OPENAL_ERROR(alSourcei(sourceID, AL_BUFFER, 0));
    idleSources.insert(sourceID);
}

void EmscriptenOpenALAudio::freeBuffer (int bufferID) {
    std::set<int>::iterator sit = idleSources.begin();
    std::set<int>::iterator send = idleSources.end();
    for (; sit != send; sit++) {
        int sourceID = *sit;
...
void EmscriptenOpenALAudio::stopSourcesWithBuffer (int bufferID) {
    std::set<int>::iterator sit = idleSources.begin();
    std::set<int>::iterator send = idleSources.end();
    for (; sit != send; sit++) {
...
void EmscriptenOpenALAudio::update () {
    for (unsigned i = 0; i < music.size(); i++)
        music[i]->update();
}

void EmscriptenOpenALAudio::dispose () {
    std::set<int>::iterator sit = idleSources.begin();
    std::set<int>::iterator send = idleSources.end();
    for (; sit != send; sit++) {
        ALuint sourceID = *sit;
...
gdx::AudioDevice* EmscriptenOpenALAudio::newAudioDevice (int samplingRate, bool isMono) {
    // BOZO - Write OpenAL device.
    return NULL;//new JavaSoundAudioDevice(samplingRate, isMono);
}

gdx::AudioRecorder* EmscriptenOpenALAudio::newAudioRecoder (int samplingRate, bool isMono) {
    return NULL;//new JavaSoundAudioRecorder(samplingRate, isMono);
}

EmscriptenOpenALAudio::~EmscriptenOpenALAudio()
{
    dispose();
}
