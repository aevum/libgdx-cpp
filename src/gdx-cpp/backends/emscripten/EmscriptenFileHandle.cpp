...
EmscriptenFileHandle::EmscriptenFileHandle (const std::string &fileName, gdx::Files::FileType type)
    : FileHandle(fileName, type)
{
}

EmscriptenFileHandle::EmscriptenFileHandle (const gdx::File &file, gdx::Files::FileType type)
    : FileHandle(file, type)
{
}

int EmscriptenFileHandle::readBytes(gdx::FileHandle::buffer_ptr& c) const
{
...
int EmscriptenFileHandle::write(const char* data, int length, bool append)
{
...
void EmscriptenFileHandle::copyTo(gdx::FileHandle& dest)
{
    gdx::FileHandle::copyTo(dest);
}

int64_t EmscriptenFileHandle::length() const
{
    struct stat st;
